...
# ---------- FLASK ДЛЯ RENDER.COM (ЧТОБЫ НЕ ЗАСЫПАЛ) ----------
...
@bot.tree.command(name="help", description="Справка по всем доступным командам", guild=discord.Object(id=GUILD_ID))
async def help_cmd(interaction: discord.Interaction):
    try:
        ...
        help_text = "\n".join(commands_list)
        embed = discord.Embed(title="📖 Справка по командам", description=help_text, color=discord.Color.blurple())
        embed.set_footer(text="SchizoBot v3.0 | Показаны только доступные тебе команды")
...
@bot.tree.command(name="info", description="Информация о боте и справка по командам", guild=discord.Object(id=GUILD_ID))
...
@bot.tree.command(name="afk", description="Установить статус АФК с причиной", guild=discord.Object(id=GUILD_ID))
@app_commands.describe(reason="Причина АФК", minutes="Сколько минут будешь в АФК? (1-1440)")
async def afk(interaction: discord.Interaction, reason: str = None, minutes: int = None):
    try:
        ...
        if minutes <= 0 or minutes > 1440:
            msg = await interaction.response.send_message("Ошибка: укажите время от 1 до 1440 минут (24 часа).", ephemeral=True)
            asyncio.create_task(bot._delete_after_custom(msg, 180))
            return
        ...
        msg = await interaction.response.send_message(f"✅ Твой АФК статус установлен на {minutes} минут.\n**Причина:** {reason}", ephemeral=True)
        asyncio.create_task(bot._delete_after_custom(msg, 180))
    except Exception as e:
        print(f"Ошибка в afk: {e}")

@bot.tree.command(name="unafk", description="Убрать себя из АФК списка", guild=discord.Object(id=GUILD_ID))
async def unafk(interaction: discord.Interaction):
    try:
        async with bot.db.execute("SELECT user_id FROM afk_users WHERE user_id = ?", (interaction.user.id,)) as cursor:
            result = await cursor.fetchone()

        if not result:
            msg = await interaction.response.send_message("Ты не находишься в АФК списке.", ephemeral=True)
            asyncio.create_task(bot._delete_after_custom(msg, 180))
            return

        await bot.db.execute("DELETE FROM afk_users WHERE user_id = ?", (interaction.user.id,))
        await bot.db.commit()

        msg = await interaction.response.send_message("✅ Ты убран из АФК списка.", ephemeral=True)
        asyncio.create_task(bot._delete_after_custom(msg, 180))
    except Exception as e:
        print(f"Ошибка в unafk: {e}")

@bot.tree.command(name="afklist", description="Список пользователей в АФК (только для админов)", guild=discord.Object(id=GUILD_ID))
@app_commands.checks.has_permissions(administrator=True)
async def afklist(interaction: discord.Interaction):
    try:
        await interaction.response.defer(ephemeral=True)
        async with bot.db.execute("SELECT user_id, reason, afk_time, return_time FROM afk_users ORDER BY return_time ASC") as cursor:
            afk_data = await cursor.fetchall()

        if not afk_data:
            embed = discord.Embed(title="📋 АФК Панель", description="В АФК никого нет!", color=discord.Color.green())
            embed.set_footer(text=f"Обновлено: {datetime.datetime.now().strftime('%H:%M:%S')}")
            msg = await interaction.followup.send(embed=embed, ephemeral=True)
            asyncio.create_task(bot._delete_after_custom(msg, 180))
            return
        ...
        for user_id, reason, afk_time, return_time in afk_data:
            try:
                ...
                user_name = member.display_name[:18] if member else (await bot.fetch_user(user_id)).name[:18]
            except:
                user_name = "Unknown"

            dt_return = datetime.datetime.fromisoformat(return_time)
            remaining = dt_return - datetime.datetime.now()
            hours = int(remaining.total_seconds() // 3600)
            mins = int((remaining.total_seconds() % 3600) // 60)
            time_left = f"{hours}ч {mins}м" if hours > 0 else f"{mins}м"
            reason_short = reason[:28] if len(reason) <= 28 else reason[:25] + "..."
        ...
        embed = discord.Embed(title="📋 АФК Панель", description="\n".join(table_lines), color=discord.Color.gold())
        embed.set_footer(text=f"Обновлено: {datetime.datetime.now().strftime('%H:%M:%S')} | Всего в АФК: {len(afk_data)}")
        msg = await interaction.followup.send(embed=embed, ephemeral=True)
        asyncio.create_task(bot._delete_after_custom(msg, 180))
    except Exception as e:
...
@bot.tree.command(name="warn", description="Выдать варн пользователю", guild=discord.Object(id=GUILD_ID))
@app_commands.checks.has_permissions(administrator=True)
@app_commands.describe(user="Пользователь для выдачи варна", reason="Причина варна")
async def warn(interaction: discord.Interaction, user: discord.User, reason: str = "Не указана"):
    try:
        if len(reason) > 200:
            msg = await interaction.response.send_message("Ошибка: причина слишком длинная (максимум 200 символов).", ephemeral=True)
            asyncio.create_task(bot._delete_after_custom(msg, 180))
            return

        now = datetime.datetime.now()
        await bot.db.execute("INSERT INTO warns (user_id, admin_id, violation_type, timestamp) VALUES (?, ?, ?, ?)",
        ...
        embed = discord.Embed(title="⚠️ Варн выдан", description=f"{user.mention} получил варн за: {reason}", color=discord.Color.red())
...
@bot.tree.command(name="warninfo", description="Посмотреть варны пользователя", guild=discord.Object(id=GUILD_ID))
...
async def warninfo(interaction: discord.Interaction, user: discord.User = None):
    try:
        ...
        async with bot.db.execute("SELECT COUNT(*) FROM warns WHERE user_id = ?", (target.id,)) as cursor:
            warn_count = (await cursor.fetchone())[0]

        async with bot.db.execute("SELECT id, admin_id, violation_type, timestamp FROM warns WHERE user_id = ? ORDER BY id DESC", (target.id,)) as cursor:
            warns = await cursor.fetchall()

        embed = discord.Embed(title=f"📋 Варны пользователя {target.name}", color=discord.Color.orange() if warn_count > 0 else discord.Color.green())
        ...
        if warns:
            ...
            for warn_id, admin_id, violation_type, timestamp in warns:
                ...
                warns_text.append(f"**ID {warn_id}** | {dt.strftime('%d.%m.%Y %H:%M')} | {admin_name}\n{violation_type}")
...
@bot.tree.command(name="dwarn", description="Удалить варн по ID", guild=discord.Object(id=GUILD_ID))
@app_commands.checks.has_permissions(administrator=True)
@app_commands.describe(warn_id="ID варна для удаления")
async def dwarn(interaction: discord.Interaction, warn_id: int):
    try:
        async with bot.db.execute("SELECT user_id, violation_type, timestamp FROM warns WHERE id = ?", (warn_id,)) as cursor:
            warn_info = await cursor.fetchone()

        if not warn_info:
            msg = await interaction.response.send_message(f"Ошибка: варн с ID {warn_id} не найден.", ephemeral=True)
            asyncio.create_task(bot._delete_after_custom(msg, 180))
            return

        user_id, violation_type, timestamp = warn_info
        await bot.db.execute("DELETE FROM warns WHERE id = ?", (warn_id,))
        await bot.db.commit()

        async with bot.db.execute("SELECT COUNT(*) FROM warns WHERE user_id = ?", (user_id,)) as cursor:
        ...
        embed = discord.Embed(title="✅ Варн удалён", color=discord.Color.green())
...
@bot.tree.command(name="ban", description="Забанить пользователя", guild=discord.Object(id=GUILD_ID))
...
async def ban(interaction: discord.Interaction, user: discord.User, reason: str = "Не указана"):
    try:
        if isinstance(user, discord.Member) and user.top_role >= interaction.user.top_role:
            msg = await interaction.response.send_message("Ошибка: вы не можете забанить пользователя с такой же или выше ролью.", ephemeral=True)
            asyncio.create_task(bot._delete_after_custom(msg, 180))
            return

        await interaction.guild.ban(user, reason=reason)
        embed = discord.Embed(title="⛔ Бан", description=f"{user.mention} забанен.\nПричина: {reason}", color=discord.Color.red())
        embed.add_field(name="Администратор", value=interaction.user.mention, inline=True)
        msg = await interaction.response.send_message(embed=embed)
        asyncio.create_task(bot._delete_after_custom(msg, 180))
        try:
...
@bot.tree.command(name="unban", description="Разбанить пользователя по ID", guild=discord.Object(id=GUILD_ID))
@app_commands.checks.has_permissions(ban_members=True)
...
async def unban(interaction: discord.Interaction, user_id: int):
    try:
        ...
        embed = discord.Embed(title="✅ Разбан", description=f"Пользователь {user.mention} разбанен.", color=discord.Color.green())
        embed.add_field(name="Администратор", value=interaction.user.mention, inline=True)
        msg = await interaction.response.send_message(embed=embed)
        asyncio.create_task(bot._delete_after_custom(msg, 180))
    except Exception as e:
...
@bot.tree.command(name="timeout", description="Выдать мут пользователю", guild=discord.Object(id=GUILD_ID))
@app_commands.checks.has_permissions(moderate_members=True)
@app_commands.describe(user="Пользователь", minutes="Длительность мута в минутах (1-40320)", reason="Причина мута")
async def timeout(interaction: discord.Interaction, user: discord.Member, minutes: int, reason: str = "Не указана"):
    try:
        if user.top_role >= interaction.user.top_role:
            msg = await interaction.response.send_message("Ошибка: вы не можете замутить пользователя с такой же или выше ролью.", ephemeral=True)
            asyncio.create_task(bot._delete_after_custom(msg, 180))
            return

        if minutes <= 0 or minutes > 40320:
            msg = await interaction.response.send_message("Ошибка: укажите время от 1 до 40320 минут (28 дней).", ephemeral=True)
            asyncio.create_task(bot._delete_after_custom(msg, 180))
            return
        ...
        embed = discord.Embed(title="🔇 Мут выдан", description=f"{user.mention} получил мут на {minutes} минут.\nПричина: {reason}", color=discord.Color.orange())
        embed.add_field(name="Администратор", value=interaction.user.mention, inline=True)
        msg = await interaction.response.send_message(embed=embed)
        asyncio.create_task(bot._delete_after_custom(msg, 180))
    except Exception as e:
...
@bot.tree.command(name="untimeout", description="Снять мут с пользователя", guild=discord.Object(id=GUILD_ID))
@app_commands.checks.has_permissions(moderate_members=True)
async def untimeout(interaction: discord.Interaction, user: discord.Member):
    try:
        if not user.is_timed_out():
            msg = await interaction.response.send_message(f"Ошибка: {user.mention} не находится в муте.", ephemeral=True)
            asyncio.create_task(bot._delete_after_custom(msg, 180))
            return

        await user.timeout(None)
        embed = discord.Embed(title="✅ Мут снят", description=f"С {user.mention} снят мут.", color=discord.Color.green())
        embed.add_field(name="Администратор", value=interaction.user.mention, inline=True)
        msg = await interaction.response.send_message(embed=embed)
        asyncio.create_task(bot._delete_after_custom(msg, 180))
    except Exception as e:
...
@bot.tree.command(name="kick", description="Выгнать пользователя", guild=discord.Object(id=GUILD_ID))
...
async def kick(interaction: discord.Interaction, user: discord.Member, reason: str = "Не указана"):
    try:
        if user.top_role >= interaction.user.top_role:
            msg = await interaction.response.send_message("Ошибка: вы не можете выгнать пользователя с такой же или выше ролью.", ephemeral=True)
            asyncio.create_task(bot._delete_after_custom(msg, 180))
            return

        await user.kick(reason=reason)
        embed = discord.Embed(title="👢 Кик", description=f"{user.mention} выгнан со сервера.\nПричина: {reason}", color=discord.Color.red())
        embed.add_field(name="Администратор", value=interaction.user.mention, inline=True)
        msg = await interaction.response.send_message(embed=embed)
        asyncio.create_task(bot._delete_after_custom(msg, 180))
        try:
...
@bot.tree.command(name="broadcast", description="Отправить сообщение в канал несколько раз (только админы)", guild=discord.Object(id=GUILD_ID))
@app_commands.checks.has_permissions(administrator=True)
